...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (sosActive) audioRef.current.play().catch(() => {});
  ...
  useEffect(() => {
    ...
    const h = e => {
      ...
      if (lX !== null) {
        ...
        const now = Date.now();
        if (d > 25 && now-last > 3000) { last=now; showNotif("📳 Shake! Calling first contact...","warning"); if (contacts.length>0) window.open(`tel:${contacts[0].phone}`); }
      }
      lX=x; lY=y; lZ=z;
    };
  ...
  const triggerSOSAlert = () => {
    ...
    if (contacts.length > 0) {
      contacts.forEach(c => {
        // Send SMS
        try { window.open(`sms:${c.phone}?body=${encodeURIComponent(msg)}`); } catch {}
        ...
        try {
          ...
          window.open(`https://wa.me/${waNum91}?text=${encodeURIComponent(msg)}`);
        } catch {}
      });
    }
  ...
  return (
    <div className="app">
        ...
        <div className="modal-overlay">
          <div className="sos-modal">
            <div className="sos-modal-top">
              ...
              <p>SMS sent to all contacts. If no signal, use options below:</p>
            </div>
              ...
              <div className="modal-contacts">
                ...
                {contacts.map(c=>(
                  <div key={c._id} className="modal-contact-row">
                    <div className="mc-info"><span className="mc-name">{c.name}</span><span className="mc-phone">{c.phone}</span></div>
                    <div className="mc-btns">
                      <button className="mc-call" onClick={()=>window.open(`tel:${c.phone}`)}>📞 Call</button>
                      <button className="mc-sms"  onClick={()=>window.open(`sms:${c.phone}?body=${encodeURIComponent(sosMessage)}`)}>💬 SMS</button>
                      <button className="mc-wa"   onClick={()=>{ const n=c.phone.replace(/[^0-9]/g,""); window.open(`https://wa.me/${n.startsWith("91")?n:"91"+n}?text=${encodeURIComponent(sosMessage)}`); }}>🟢 WA</button>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="modal-warn"><p>⚠️ No contacts saved! Add them in Contacts tab.</p><button className="copy-btn" onClick={()=>{setSosModal(false);setActiveTab("contacts");}}>Add Contacts</button></div>
            )}
      ...
      {/* Fake call overlay — separate from modal-overlay to avoid z-index conflicts */}
      {fakeActive && (
        <div className="fakecall-overlay">
          <div className="fakecall-card">
            {/* Ripple rings — pointer-events:none so they never block buttons */}
            <div className="fc-ripple"/>
            <div className="fc-ripple fc-ripple-2"/>
            <p className="fc-label">Incoming Call</p>
            <div className="fc-avatar">{fakeCaller[0]}</div>
            <p className="fc-name">{fakeCaller}</p>
            <p className="fc-sub">Mobile · SaveHer App</p>
            <div className="fc-btns">
              <div className="fc-btn-wrap">
                <button
                  className="fc-decline"
                  onPointerDown={(e)=>{ e.stopPropagation(); endFakeCall(); }}
                >
                  📵
                </button>
                <span className="fc-btn-lbl">Decline</span>
              </div>
              <div className="fc-btn-wrap">
                <button
                  className="fc-accept"
                ...
                <span className="fc-btn-lbl">Accept</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="countdown-wrap">
            <svg className="countdown-svg" viewBox="0 0 100 100">
              ...
              <circle cx="50" cy="50" r="44" fill="none" stroke="#e11d48" strokeWidth="6" strokeLinecap="round"
                strokeDasharray={`${(sosCountdown/5)*276.5} 276.5`} strokeDashoffset="69.1" style={{transition:"stroke-dasharray 1s linear"}}/>
            </svg>
      ...
      <nav className="tabs">
        {[["home","🏠","Home"],["contacts","👥","Contacts"],["location","📍","Location"],["recorder","🎤","Recorder"],["safety","🛡️","Safety"]].map(([t,ic,lb])=>(
          <button key={t} className={`tab ${activeTab===t?"tab-on":""}`} onClick={()=>setActiveTab(t)}>
            <span className="tab-ic">{ic}</span><span className="tab-lb">{lb}</span>
          </button>
        ))}
      </nav>
      ...
      <div className="content">
          ...
          <div className="pg">
            ...
            <div className="grid3">
              {[
                ...
                ["📍","Location","GPS",fetchLocation],
                ["🚔","Police","100",()=>window.open("tel:100")],
                ["🆘","Women","1091",()=>window.open("tel:1091")],
                ["🚑","Ambulance","102",()=>window.open("tel:102")],
            ...
            <div className="grid3">
              {[["🚔","Police","100"],["👩","Women","1091"],["🚑","Ambulance","102"],["🔥","Fire","101"],["🆘","Disaster","108"],["🏥","Child","1098"]].map(([ic,lb,num])=>(
                <button key={num} className="ebtn" onClick={()=>window.open(`tel:${num}`)}>
            ...
            <div className="stat-row">
              <div className="stat-box">
                ...
                <div className="risk-bar"><div className="risk-fill" style={{width:`${timeRisk.pct}%`,background:timeRisk.color}}/></div>
              </div>
              <div className="stat-box"><span className="stat-v">{contacts.length}</span><span className="stat-l">Contacts Saved</span></div>
              <div className="stat-box"><span className="stat-v">{recordings.length}</span><span className="stat-l">Recordings</span></div>
            </div>
          </div>
        )}
        ...
        {activeTab==="contacts" && (
          <div className="pg">
            ...
            <p className="pg-desc">Saved in browser storage. Automatically notified when SOS activates.</p>
            <div className="form-stack">
              ...
              <select className="inp" value={cRelation} onChange={e=>setCRelation(e.target.value)}>
                <option>Family</option><option>Friend</option><option>Neighbor</option><option>Colleague</option>
              </select>
            ...
            {contacts.length===0
              ...
              : contacts.map(c=>(
                <div key={c._id} className="c-card">
                  ...
                  <div className="c-info"><p className="c-name">{c.name}</p><p className="c-meta">{c.phone} · <span className="c-rel">{c.relation}</span></p></div>
                  <div className="c-acts">
                    <button className="ic-btn" onClick={()=>window.open(`tel:${c.phone}`)}>📞</button>
                    <button className="ic-btn" onClick={()=>window.open(`sms:${c.phone}`)}>💬</button>
        ...
        {activeTab==="location" && (
          <div className="pg">
            ...
            <div className="loc-mode-box">
              ...
              {locationMode==="manual" && (
                <div className="manual-loc-wrap">
                  <p className="manual-loc-label">Choose your city — distances will be calculated from here:</p>
                  ...
                  <p className="manual-loc-hint">💡 Perfect for laptop demo — picks exact city coordinates</p>
                </div>
              )}

              {locationMode==="gps" && (
                <div className="btn-col">
                  <button className={`btn-primary ${isTracking?"btn-danger":"btn-success"}`} onClick={toggleTrack}>{isTracking?"🔴 Stop Live Tracking":"🟢 Start Live Tracking"}</button>
                </div>
              )}
            </div>
            ...
            {location && location.manual && (
              <div className="loc-manual-badge">
                🏙️ Manual city: <strong>{location.cityName}</strong> — {location.latitude.toFixed(4)}, {location.longitude.toFixed(4)}
              </div>
            )}
            {location && !location.manual && <div className="gps-ok">✅ Real GPS active — {location.latitude.toFixed(5)}, {location.longitude.toFixed(5)}</div>}
            {isTracking && <div className="live-badge"><span className="live-dot"/>Live GPS tracking active</div>}
            ...
            {location && (
              <>
                ...
                {showMap && <div className="map-wrap"><div id="loc-map" className="leaflet-map"/><div className="map-coords">📍 {location.latitude.toFixed(5)}, {location.longitude.toFixed(5)}</div></div>}
                <div className="loc-card">
                  {[["Latitude",location.latitude.toFixed(6)],["Longitude",location.longitude.toFixed(6)],["Source",location.manual?"Manual ("+location.cityName+")":"Real GPS"]].map(([l,v])=>(
                    <div key={l} className="loc-row"><span className="loc-l">{l}</span><span className="loc-v" style={{fontSize:l==="Source"?11:13}}>{v}</span></div>
                  ))}
                  ...
                  {contacts.length>0 && <button className="share-btn" onClick={()=>{ const m=`My location: https://maps.google.com/?q=${location.latitude},${location.longitude}`; contacts.forEach(c=>window.open(`sms:${c.phone}?body=${encodeURIComponent(m)}`)); showNotif("📤 Shared!"); }}>📤 Share with All Contacts</button>}
                </div>
              </>
            )}
          ...
          <div className="pg">
            ...
            {recordings.length===0 && !recording && (
              <div className="empty">
                ...
                <small>Recordings survive app close — stored in your device permanently</small>
              </div>
            )}
                ...
                {recordings.map(r => (
                  <div key={r.id} className="rec-card-full">
                    ...
                    <div className="rec-card-acts">
                      {/* Download */}
                      <a href={r.url} download={`evidence-${r.id}.webm`} className="rec-act-btn dl-btn">
                        ⬇️ Download
                      </a>
                      ...
                      {!uploadUrls[r.id] ? (
                        <button
                          className="rec-act-btn upload-btn"
                          onClick={() => uploadRecording(r)}
                          disabled={uploading[r.id]}
                        >
                          {uploading[r.id] ? "⏳ Uploading…" : "☁️ Upload to Cloud"}
                        </button>
                      ) : (
                        <button className="rec-act-btn uploaded-btn" disabled>
                          ✅ Uploaded
                        </button>
                      )}

                      {/* Delete */}
                      <button className="rec-act-btn del-btn" onClick={() => deleteRec(r.id)}>
                        🗑️ Delete
                      </button>
                    </div>

                    {/* Show cloud URL if uploaded */}
                    {uploadUrls[r.id] && (
                      <div className="cloud-url-box">
                        ...
                        <button className="copy-btn" onClick={() => {
                          navigator.clipboard?.writeText(uploadUrls[r.id]).then(() => showNotif("📋 Cloud link copied!"));
                        }}>Copy Link</button>
                      </div>
                    )}
                  </div>
            ...
            <div className="cloud-setup-box">
              ...
              <p className="cloud-setup-desc">
                To enable cloud upload: Sign up free at <strong>cloudinary.com</strong> →
                Get your Cloud Name → Replace <code>saveher2024</code> in Home.js line ~20
              </p>
            </div>
          </div>
        )}
        ...
        {activeTab==="safety" && (
          <div className="pg">
            ...
            {/* ── Always-visible city selector — works offline, no GPS needed ── */}
            <div className="city-from-box">
              <span className="city-from-label">📍 Your City:</span>
              <select
                className="inp city-from-select"
                value={manualCity}
            ...
            <div className="city-from-badge">
              ✅ Distances calculated from <strong>{location?.cityName || manualCity}</strong> — change anytime above
            </div>

            <div className="search-row">
              <input className="inp" placeholder="e.g. madurai, coimbatore…" value={areaInput} onChange={e=>setAreaInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&searchArea()}/>
              <button className="btn-primary srch-btn" onClick={()=>searchArea()}>Search</button>
            </div>

            <div className="chips">
              {Object.keys(TN_AREAS).map(a=>(
                <button key={a} className={`chip ${selArea===a?"chip-on":""}`} onClick={()=>{setAreaInput(a);searchArea(a);}}>
            ...
            {areaResult && (
              <div className="safety-card">
                ...
                <div className="trend-section">
                  <p className="sec-label">Crimes Against Women — Monthly Trend 2022</p>
                ...
                {/* Offline map */}
                <div className="sec-label" style={{marginBottom:6}}>📍 Area Map (Offline — OpenStreetMap)</div>
                <div className="map-wrap" style={{marginBottom:8}}>
                ...
                <textarea className="inp" placeholder="Describe what happened… Your report helps others stay safe." value={areaReport} onChange={e=>setAreaReport(e.target.value)} rows={3} style={{resize:"vertical",marginBottom:8}}/>
                  ...
                  <div className="reports">
                    ...
                    {areaReports.map(r=>(
                      <div key={r.id} className="r-item">
                        ...
                        <div className="r-foot"><span className="r-time">{r.time}</span><button className="r-del" onClick={()=>delReport(selArea,r.id)}>🗑️</button></div>
                      </div>
                    ))}
                  </div>
                )}
